...
- [x] Unit coverage confirmed/extended — confirmed, no gaps to extend (see Notes):
  ...
  - [x] SSRF URL guard (allow/deny table) *(fetch_resource_spec.rb — 11-row deny table asserting `:rejected_url` + zero requests, plus bracket normalization and redirect re-guarding; enrichment_queuer_spec.rb — hostile-URL fixture at ingest. Deliberately no dedicated url_guard_spec — see Notes)*
- [x] Integration spec: full ingest cycle against WebMock'd fixtures (captured real `/events` JSON) → asserts raw rows, structured rows, enqueued jobs, log events *(spec/integration/ingest_cycle_spec.rb — real GithubClient + EventIngester + EnrichmentQueuer through `IngestRunner#run(once: true)`; content-determined counts derive from the fixture, while the zero overlays — no malformed, no duplicates — and D-022's 120s floor are deliberate literals: the zeros are properties of this capture a re-capture may need to revisit, and the floor is the suite's one pin of the policy value, sourced from D-022 rather than the constant under test per D-025)*
...
- [x] Length check: ≤2 pages. Cut ruthlessly; link to docs/ARCHITECTURE.md for depth. *(~1,050 words including the diagram; a "What I Tested and Why" section was added so Extension D has a brief-section anchor, per the exit criterion's traceability requirement)*
...
- [x] DESIGN.md covers all five required brief topics in ≤2 pages *(verified — word count and the Extension-D anchor recorded at the length-check task above)*
...
- **Pre-merge review (same ritual as Phases 2–4) confirmed 10 findings, all remediated on this branch.** The two that mattered: DESIGN.md claimed the restart spec asserts "byte-identical database state" while the spec deliberately excludes the upsert-bumped timestamps — falsehood #9 of the docs defect class, caught before it shipped in the one document that gets submitted; and no spec in the suite pinned the 120s floor as a literal (every assertion sourced `IngestRunner::POLL_FLOOR`, D-025's expectation-moves-with-the-mutant anti-pattern) — the integration spec now carries the suite's one literal pin of D-022's policy value. Also from the review: entity identity is now anchored to the fixture's ids (counts alone could not catch a consistently garbled seam), the restart snapshot flipped from column inclusion-lists to exclusion (`attributes.except`) so future columns are compared by default, `RecordingLogger#messages` grew the shared `event:` filter that five files had been hand-rolling (that round migrated three of them; the second pass below finished the job), and two redundant assertions plus this plan's own duplicated claims were removed. Suite re-run green after remediation.
- **A second review pass over the finished branch confirmed six more findings, all remediated.** The two that mattered: the enqueued-job identity anchor covered only the actor half — `EnrichRepositoryJob` arguments were tallied but never matched against row ids, so the wrong-pairing bug class the anchor exists for could still ship on the repository leg (now pinned for both job classes); and the previous bullet originally claimed the `event:` filter replaced hand-rolling in "four files" when five files hand-rolled it and only three had been migrated — falsehood #10 of the docs defect class, in the very sentence describing remediation #9's round; `enrichment_sweep_spec` and `enrichment_fetcher_spec` now delegate too, making `recording_logger.rb`'s "every spec filters the same way" comment true. Also: `messages(event:)` filters out non-Hash log lines instead of raising `TypeError` from `String#[]`; the restart example no longer re-pins run one's cycle tuple (the narration example owns it — one cause, one failure); `REPLAY_MUTABLE` became a method because a constant assigned inside a `describe` block is defined on `Object` and leaks suite-wide; the queuer spec's one-line `entries` wrapper was inlined in favor of the shared vocabulary. Suite re-run green after remediation.
