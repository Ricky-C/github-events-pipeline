...
- **Auth tokens** — an explicit exercise constraint, honored strictly; no token support even for convenience.
...
- **Multi-instance coordination** — single-node by design; the budget is per-IP, so horizontal scale buys nothing without more egress IPs or authentication.
